...
IP_LIST_FILE="/etc/ansible/stage_ips.txt"        # Temporary file for discovered IPs
...
DOCKER_USER="admin"                              # Docker username
DOCKER_PASSWORD="admin123"                       # Docker password
APP_IMAGE="$DOCKER_REPO/apppetclinic:latest"     # Docker image name/tag
...
check_docker_container() {
    echo "🐳 Checking Docker container status on all instances..."
    while read -r ip; do
        [ -z "$ip" ] && continue
