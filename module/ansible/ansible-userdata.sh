...
sudo aws s3 cp s3://"${s3_bucket}"/scripts/ /etc/ansible/ --recursive
...
sudo chmod 755 /etc/ansible/prod_bash.sh
sudo chmod 755 /etc/ansible/stage_bash.sh

# Create cron job using our bash script
echo "* * * * * ec2-user /etc/ansible/prod_bash.sh" | sudo tee -a /etc/crontab
echo "* * * * * ec2-user /etc/ansible/stage_bash.sh" | sudo tee -a /etc/crontab
